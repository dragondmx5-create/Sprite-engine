...
var SpriteEngine = (() => {
  ...
  function buildStoneWall(rng, s) {
    ...
    pushBox2(parts, MATERIALS.bone(base), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.3);
    const my = s * (0.38 + rng.jitter(0.04));
    pushBox2(parts, MATERIALS.bone([base[0] * 0.6, base[1] * 0.6, base[2] * 0.6]), s * 0.5, my, s * 0.44, s * 0.012, s * 5e-3, 0.08);
    const mx = s * (0.33 + rng.jitter(0.06));
    pushBox2(parts, MATERIALS.bone([base[0] * 0.63, base[1] * 0.63, base[2] * 0.63]), mx, s * 0.22, s * 0.012, s * 0.16, s * 5e-3, 0.08);
    const mx2 = s * (0.6 + rng.jitter(0.06));
    pushBox2(parts, MATERIALS.bone([base[0] * 0.63, base[1] * 0.63, base[2] * 0.63]), mx2, s * 0.68, s * 0.012, s * 0.16, s * 5e-3, 0.08);
    return parts;
  }
  ...
  function buildWoodDoor(rng, s) {
    ...
    const frame = [78, 73, 68];
    pushBox2(parts, MATERIALS.bone(frame), s * 0.5, s * 0.5, s * 0.48, s * 0.48, s * 0.02, 0.2);
    pushBox2(parts, MATERIALS.leather(wood), s * 0.5, s * 0.52, s * 0.34, s * 0.42, s * 0.025, 0.28);
    const band = MATERIALS.metal([95, 95, 105]);
    pushBox2(parts, band, s * 0.5, s * 0.28, s * 0.36, s * 0.016, s * 8e-3, 0.35);
    pushBox2(parts, band, s * 0.5, s * 0.72, s * 0.36, s * 0.016, s * 8e-3, 0.35);
    pushCircle3(parts, MATERIALS.metal([148, 138, 96]), s * 0.63, s * 0.52, s * 0.028, 0.55);
    return parts;
  }
  ...
  function buildSpikeTrap(rng, s) {
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        pushCapsule3(parts, spikeMat, sx, sy + h * 0.5, sx, sy - h * 0.5, Math.max(1, s * 0.016), 0.6);
      }
    }
    return parts;
  }
  ...
  function buildCrackedWall(rng, s) {
    ...
    pushBox2(parts, MATERIALS.bone(base), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.28);
    const crackCol = MATERIALS.bone([base[0] * 0.4, base[1] * 0.4, base[2] * 0.4]);
    ...
    for (let i = 0; i < cracks; i++) {
      ...
      pushCapsule3(parts, crackCol, ax, ay, bx, by, Math.max(1, s * 0.01), 0.1);
    }
    const my = s * (0.35 + rng.jitter(0.05));
    pushBox2(parts, MATERIALS.bone([base[0] * 0.55, base[1] * 0.55, base[2] * 0.55]), s * 0.5, my, s * 0.44, s * 0.01, s * 4e-3, 0.08);
    return parts;
  }
  ...
  function buildStalagmite(rng, s) {
    ...
    for (let i = 0; i < pillars; i++) {
      ...
      pushCapsule3(parts, MATERIALS.bone(col), cx, baseY, cx + rng.jitter(s * 0.03), tipY, thickness, 0.25);
    }
    return parts;
  }
  ...
  function buildBarrel(rng, s) {
    ...
    pushBox2(parts, MATERIALS.leather(woodCol), s * 0.5, s * 0.5, s * 0.22, s * 0.32, s * 0.12, 0.3);
    const bandMat = MATERIALS.metal([140, 135, 128]);
    pushBox2(parts, bandMat, s * 0.5, s * 0.3, s * 0.24, s * 0.018, s * 8e-3, 0.4);
    pushBox2(parts, bandMat, s * 0.5, s * 0.7, s * 0.24, s * 0.018, s * 8e-3, 0.4);
    return parts;
  }
  ...
  function buildShopCounter(rng, s) {
    ...
    pushBox2(parts, MATERIALS.leather(woodCol), s * 0.5, s * 0.2, s * 0.38, s * 0.14, s * 0.02, 0.22);
    pushBox2(parts, MATERIALS.leather([woodCol[0] * 0.8, woodCol[1] * 0.8, woodCol[2] * 0.8]), s * 0.35, s * 0.2, s * 0.012, s * 0.12, s * 5e-3, 0.18);
    pushBox2(parts, MATERIALS.leather([woodCol[0] * 0.8, woodCol[1] * 0.8, woodCol[2] * 0.8]), s * 0.65, s * 0.2, s * 0.012, s * 0.12, s * 5e-3, 0.18);
    pushBox2(parts, MATERIALS.leather(woodCol), s * 0.5, s * 0.58, s * 0.4, s * 0.08, s * 0.03, 0.28);
    pushBox2(parts, MATERIALS.leather([woodCol[0] * 0.85, woodCol[1] * 0.85, woodCol[2] * 0.85]), s * 0.5, s * 0.72, s * 0.38, s * 0.1, s * 0.02, 0.2);
    pushCircle3(parts, MATERIALS.gold([210, 185, 80]), s * 0.58, s * 0.55, s * 0.035, 0.5);
    pushCircle3(parts, MATERIALS.gold([195, 170, 70]), s * 0.54, s * 0.56, s * 0.025, 0.45);
    return parts;
  }
  function buildIronGate(rng, s) {
    const parts = [];
    const frameCol = [52 + rng.jitter(6), 48 + rng.jitter(5), 46 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.bone(frameCol), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.25);
    pushBox2(parts, MATERIALS.bone([18, 15, 12]), s * 0.5, s * 0.5, s * 0.34, s * 0.42, s * 0.01, 0.08);
    pushBox2(parts, MATERIALS.bone(frameCol), s * 0.5, s * 0.06, s * 0.44, s * 0.06, s * 0.02, 0.3);
    pushBox2(parts, MATERIALS.bone(frameCol), s * 0.5, s * 0.94, s * 0.44, s * 0.06, s * 0.02, 0.3);
    const barMat = MATERIALS.metal([120 + rng.jitter(8), 118 + rng.jitter(6), 115 + rng.jitter(6)]);
    const bars = 5;
    for (let i = 0; i < bars; i++) {
      const bx = s * (0.24 + i * 0.13);
      pushCapsule3(parts, barMat, bx, s * 0.12, bx, s * 0.88, Math.max(1.2, s * 0.02), 0.45);
    }
    pushCapsule3(parts, barMat, s * 0.18, s * 0.5, s * 0.82, s * 0.5, Math.max(1.2, s * 0.016), 0.4);
    return parts;
  }
  function buildTorchBracket(rng, s) {
    const parts = [];
    const wallCol = [62 + rng.jitter(8), 58 + rng.jitter(6), 56 + rng.jitter(6)];
    pushBox2(parts, MATERIALS.bone(wallCol), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.28);
    const mortarCol = [wallCol[0] * 0.6, wallCol[1] * 0.6, wallCol[2] * 0.6];
    pushBox2(parts, MATERIALS.bone(mortarCol), s * 0.5, s * 0.45, s * 0.44, s * 0.01, s * 4e-3, 0.08);
    const bracketMat = MATERIALS.metal([130, 125, 120]);
    pushCapsule3(parts, bracketMat, s * 0.5, s * 0.35, s * 0.5, s * 0.65, Math.max(1.2, s * 0.022), 0.4);
    pushCapsule3(parts, bracketMat, s * 0.5, s * 0.38, s * 0.68, s * 0.38, Math.max(1.2, s * 0.02), 0.4);
    const torchWood = [100, 62, 35];
    pushCapsule3(parts, MATERIALS.leather(torchWood), s * 0.68, s * 0.22, s * 0.68, s * 0.4, Math.max(1.2, s * 0.025), 0.25);
    pushEllipse3(parts, MATERIALS.ember([255, 180, 40]), s * 0.68, s * 0.15, s * 0.045, s * 0.07, 0.7);
    pushEllipse3(parts, MATERIALS.ember([255, 230, 120]), s * 0.68, s * 0.16, s * 0.025, s * 0.04, 0.8);
    return parts;
  }
  function buildAltar(rng, s) {
    ...
    pushBox2(parts, MATERIALS.bone(stoneCol), s * 0.5, s * 0.72, s * 0.32, s * 0.06, s * 0.02, 0.2);
    pushBox2(parts, MATERIALS.bone(stoneCol), s * 0.5, s * 0.52, s * 0.28, s * 0.16, s * 0.03, 0.25);
    pushBox2(parts, MATERIALS.bone([stoneCol[0] + 15, stoneCol[1] + 12, stoneCol[2] + 18]), s * 0.5, s * 0.38, s * 0.3, s * 0.03, s * 0.015, 0.2);
    const runeCol = [140 + rng.jitter(20), 80 + rng.jitter(15), 200 + rng.jitter(20)];
    pushCircle3(parts, MATERIALS.gem(runeCol), s * 0.5, s * 0.48, s * 0.1, 0.6);
    pushCircle3(parts, MATERIALS.ember([180, 120, 255]), s * 0.5, s * 0.48, s * 0.05, 0.8);
    return parts;
  }
  function buildAnvil(rng, s) {
    ...
    pushBox2(parts, MATERIALS.metal(anvilCol), s * 0.45, s * 0.7, s * 0.12, s * 0.08, s * 0.02, 0.35);
    pushBox2(parts, MATERIALS.metal(anvilCol), s * 0.45, s * 0.58, s * 0.08, s * 0.06, s * 0.015, 0.3);
    pushBox2(parts, MATERIALS.metal([anvilCol[0] + 15, anvilCol[1] + 12, anvilCol[2] + 10]), s * 0.45, s * 0.45, s * 0.18, s * 0.06, s * 0.025, 0.4);
    pushCapsule3(parts, MATERIALS.metal(anvilCol), s * 0.25, s * 0.44, s * 0.15, s * 0.44, Math.max(1.2, s * 0.03), 0.38);
    const hammerHead = [100, 95, 90];
    pushBox2(parts, MATERIALS.metal(hammerHead), s * 0.72, s * 0.58, s * 0.04, s * 0.03, s * 0.01, 0.4);
    pushCapsule3(parts, MATERIALS.leather([90, 58, 35]), s * 0.72, s * 0.62, s * 0.72, s * 0.78, Math.max(1.2, s * 0.014), 0.22);
    return parts;
  }
  function buildBed(rng, s) {
    ...
    pushBox2(parts, MATERIALS.leather(frameCol), s * 0.5, s * 0.55, s * 0.36, s * 0.22, s * 0.025, 0.2);
    pushBox2(parts, MATERIALS.leather([frameCol[0] * 0.9, frameCol[1] * 0.9, frameCol[2] * 0.9]), s * 0.5, s * 0.3, s * 0.36, s * 0.04, s * 0.02, 0.25);
    const blanketCol = [65 + rng.jitter(15), 55 + rng.jitter(10), 80 + rng.jitter(15)];
    pushBox2(parts, MATERIALS.cloth(blanketCol), s * 0.5, s * 0.58, s * 0.32, s * 0.16, s * 0.03, 0.15);
    const pillowCol = [160 + rng.jitter(10), 155 + rng.jitter(8), 145 + rng.jitter(8)];
    pushEllipse3(parts, MATERIALS.cloth(pillowCol), s * 0.5, s * 0.38, s * 0.1, s * 0.04, 0.2);
    return parts;
  }
  function buildTable(rng, s) {
    ...
    const floorCol = [78 + rng.jitter(6), 72 + rng.jitter(5), 68 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.bone(floorCol), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.12);
    const legCol = [88 + rng.jitter(6), 58 + rng.jitter(5), 35 + rng.jitter(4)];
    pushCapsule3(parts, MATERIALS.leather(legCol), s * 0.28, s * 0.52, s * 0.28, s * 0.78, Math.max(1.2, s * 0.02), 0.2);
    pushCapsule3(parts, MATERIALS.leather(legCol), s * 0.72, s * 0.52, s * 0.72, s * 0.78, Math.max(1.2, s * 0.02), 0.2);
    const topCol = [110 + rng.jitter(8), 72 + rng.jitter(6), 45 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.leather(topCol), s * 0.5, s * 0.48, s * 0.34, s * 0.05, s * 0.025, 0.28);
    pushBox2(parts, MATERIALS.bone([140, 130, 115]), s * 0.58, s * 0.42, s * 0.03, s * 0.04, s * 0.015, 0.3);
    pushCircle3(parts, MATERIALS.bone([120, 108, 90]), s * 0.58, s * 0.39, s * 0.028, 0.2);
    return parts;
  }
  function buildBookshelf(rng, s) {
    ...
    pushBox2(parts, MATERIALS.leather(shelfCol), s * 0.5, s * 0.5, s * 0.38, s * 0.42, s * 0.02, 0.22);
    pushBox2(parts, MATERIALS.leather([shelfCol[0] * 0.65, shelfCol[1] * 0.65, shelfCol[2] * 0.65]), s * 0.5, s * 0.5, s * 0.34, s * 0.38, s * 0.01, 0.15);
    pushBox2(parts, MATERIALS.leather(shelfCol), s * 0.5, s * 0.35, s * 0.36, s * 0.015, s * 6e-3, 0.2);
    pushBox2(parts, MATERIALS.leather(shelfCol), s * 0.5, s * 0.6, s * 0.36, s * 0.015, s * 6e-3, 0.2);
    const bookColors = [
    ...
    for (let i = 0; i < 4; i++) {
      ...
      pushBox2(parts, MATERIALS.cloth(bookColors[i]), bx, s * 0.26 - bh * 0.3, s * 0.04, bh, s * 8e-3, 0.15);
    }
    for (let i = 0; i < 3; i++) {
      const bx = s * (0.25 + i * 0.16) + rng.jitter(s * 0.01);
      const bh = s * (0.06 + rng.float() * 0.04);
      const col = [80 + rng.jitter(40), 60 + rng.jitter(30), 50 + rng.jitter(30)];
      pushBox2(parts, MATERIALS.cloth(col), bx, s * 0.51 - bh * 0.3, s * 0.045, bh, s * 8e-3, 0.15);
    }
    return parts;
  }
  function buildPillar(rng, s) {
    ...
    pushBox2(parts, MATERIALS.bone([pillarCol[0] + 10, pillarCol[1] + 8, pillarCol[2] + 6]), s * 0.5, s * 0.85, s * 0.18, s * 0.05, s * 0.02, 0.2);
    pushCapsule3(parts, MATERIALS.bone(pillarCol), s * 0.5, s * 0.18, s * 0.5, s * 0.8, Math.max(1.2, s * 0.12), 0.35);
    pushBox2(parts, MATERIALS.bone([pillarCol[0] + 15, pillarCol[1] + 12, pillarCol[2] + 10]), s * 0.5, s * 0.14, s * 0.18, s * 0.05, s * 0.02, 0.22);
    return parts;
  }
  function buildFountain(rng, s) {
    ...
    pushBox2(parts, MATERIALS.bone(basinCol), s * 0.5, s * 0.55, s * 0.32, s * 0.22, s * 0.1, 0.22);
    pushCircle3(parts, MATERIALS.bone([basinCol[0] * 0.6, basinCol[1] * 0.6, basinCol[2] * 0.6]), s * 0.5, s * 0.55, s * 0.22, 0.15);
    const waterCol = [40 + rng.jitter(8), 75 + rng.jitter(10), 110 + rng.jitter(10)];
    pushCircle3(parts, MATERIALS.glass(waterCol), s * 0.5, s * 0.55, s * 0.18, 0.2);
    pushCapsule3(parts, MATERIALS.bone([basinCol[0] + 8, basinCol[1] + 6, basinCol[2] + 5]), s * 0.5, s * 0.25, s * 0.5, s * 0.55, Math.max(1.2, s * 0.03), 0.25);
    pushCircle3(parts, MATERIALS.glass([waterCol[0] + 40, waterCol[1] + 50, waterCol[2] + 40]), s * 0.5, s * 0.22, s * 0.035, 0.3);
    return parts;
  }
