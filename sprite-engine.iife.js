...
var SpriteEngine = (() => {
  ...
  function buildStoneFloor(rng, s) {
    ...
    for (let row = 0; row < 2; row++) {
      for (let col = 0; col < 2; col++) {
        ...
        const base = [88 + j, 82 + j, 76 + j];
        const cx = gap + bw / 2 + col * (bw + gap);
        const cy = gap + bh / 2 + row * (bh + gap);
        pushBox2(parts, MATERIALS.bone(base), cx, cy, bw / 2 - 1, bh / 2 - 1, s * 0.02, 0.15);
      }
    }
    if (rng.float() > 0.5) {
      const ax = s * (0.2 + rng.float() * 0.6), ay = s * (0.2 + rng.float() * 0.6);
      const bx = ax + rng.jitter(s * 0.2), by = ay + rng.jitter(s * 0.2);
      pushCapsule3(parts, MATERIALS.bone([50, 46, 42]), ax, ay, bx, by, Math.max(1, s * 8e-3), 0.1);
    }
    return parts;
  }
  function buildDirtFloor(rng, s) {
    const parts = [];
    const base = [110 + rng.jitter(15), 80 + rng.jitter(10), 55 + rng.jitter(10)];
    pushBox2(parts, MATERIALS.flesh(base), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.03, 0.08);
    for (let i = 0; i < 3; i++) {
      const px = s * (0.15 + rng.float() * 0.7);
      const py = s * (0.15 + rng.float() * 0.7);
      const pr = s * (0.025 + rng.float() * 0.02);
      pushCircle3(parts, MATERIALS.bone([base[0] * 0.75, base[1] * 0.75, base[2] * 0.75]), px, py, pr, 0.25);
    }
    return parts;
  }
  function buildStoneWall(rng, s) {
    const parts = [];
    const base = [62 + rng.jitter(8), 58 + rng.jitter(6), 56 + rng.jitter(6)];
    pushBox2(parts, MATERIALS.bone(base), s * 0.5, s * 0.5, s * 0.48, s * 0.48, s * 0.02, 0.3);
    const edgeDark = [base[0] * 0.5, base[1] * 0.5, base[2] * 0.5];
    pushBox2(parts, MATERIALS.bone(edgeDark), s * 0.97, s * 0.5, s * 0.02, s * 0.48, s * 5e-3, 0.15);
    pushBox2(parts, MATERIALS.bone(edgeDark), s * 0.5, s * 0.97, s * 0.48, s * 0.02, s * 5e-3, 0.15);
    const edgeLight = [base[0] * 1.2, base[1] * 1.2, base[2] * 1.2];
    pushBox2(parts, MATERIALS.bone(edgeLight), s * 0.03, s * 0.5, s * 0.02, s * 0.48, s * 5e-3, 0.18);
    pushBox2(parts, MATERIALS.bone(edgeLight), s * 0.5, s * 0.03, s * 0.48, s * 0.02, s * 5e-3, 0.18);
    const lighter = [base[0] + 8, base[1] + 6, base[2] + 5];
    pushBox2(parts, MATERIALS.bone(lighter), s * 0.3, s * 0.3, s * 0.18, s * 0.18, s * 0.01, 0.25);
    pushBox2(parts, MATERIALS.bone(lighter), s * 0.72, s * 0.68, s * 0.16, s * 0.2, s * 0.01, 0.25);
    return parts;
  }
  function buildCrystalFloor(rng, s) {
    const parts = [];
    const base = [38 + rng.jitter(8), 33 + rng.jitter(6), 48 + rng.jitter(8)];
    pushBox2(parts, MATERIALS.bone(base), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.12);
    ...
    for (let i = 0; i < veins; i++) {
      ...
      const color = [70 + rng.jitter(20), 150 + rng.jitter(30), 200 + rng.jitter(20)];
      pushCapsule3(parts, MATERIALS.gem(color), ax, ay, bx, by, r, 0.5);
    }
    return parts;
  }
  function buildWoodDoor(rng, s) {
    const parts = [];
    const frame = [62 + rng.jitter(6), 58 + rng.jitter(5), 56 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.bone(frame), s * 0.12, s * 0.5, s * 0.12, s * 0.48, s * 0.02, 0.28);
    pushBox2(parts, MATERIALS.bone(frame), s * 0.88, s * 0.5, s * 0.12, s * 0.48, s * 0.02, 0.28);
    const floorCol = [78 + rng.jitter(6), 72 + rng.jitter(5), 68 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.bone(floorCol), s * 0.5, s * 0.5, s * 0.28, s * 0.48, s * 0.02, 0.12);
    const wood = [118 + rng.jitter(10), 78 + rng.jitter(8), 48 + rng.jitter(6)];
    pushBox2(parts, MATERIALS.leather(wood), s * 0.5, s * 0.5, s * 0.26, s * 0.06, s * 0.02, 0.28);
    const band = MATERIALS.metal([95, 95, 105]);
    pushBox2(parts, band, s * 0.34, s * 0.5, s * 0.012, s * 0.07, s * 5e-3, 0.35);
    pushBox2(parts, band, s * 0.66, s * 0.5, s * 0.012, s * 0.07, s * 5e-3, 0.35);
    pushCircle3(parts, MATERIALS.metal([148, 138, 96]), s * 0.56, s * 0.5, s * 0.02, 0.55);
    return parts;
  }
  function buildLavaFloor(rng, s) {
    const parts = [];
    const base = [38 + rng.jitter(5), 26 + rng.jitter(4), 22 + rng.jitter(4)];
    pushBox2(parts, MATERIALS.bone(base), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.1);
    const veins = 2 + Math.floor(rng.float() * 2);
    for (let i = 0; i < veins; i++) {
      ...
      const color = [255, 100 + rng.jitter(40), 30 + rng.jitter(20)];
      pushCapsule3(parts, MATERIALS.ember(color), ax, ay, bx, by, r, 0.4);
    }
    return parts;
  }
  function buildIceFloor(rng, s) {
    const parts = [];
    const base = [168 + rng.jitter(10), 198 + rng.jitter(8), 218 + rng.jitter(6)];
    pushBox2(parts, MATERIALS.glass(base), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.15);
    for (let i = 0; i < 2; i++) {
      const ax = s * (0.15 + rng.float() * 0.7), ay = s * (0.15 + rng.float() * 0.7);
      const bx = s * (0.15 + rng.float() * 0.7), by = s * (0.15 + rng.float() * 0.7);
      pushCapsule3(parts, MATERIALS.glass([220, 235, 250]), ax, ay, bx, by, Math.max(1, s * 8e-3), 0.2);
    }
    return parts;
  }
  function buildMossFloor(rng, s) {
    const parts = [];
    const base = [62 + rng.jitter(8), 76 + rng.jitter(8), 52 + rng.jitter(6)];
    pushBox2(parts, MATERIALS.flesh(base), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.08);
    for (let i = 0; i < 3; i++) {
      const px = s * (0.15 + rng.float() * 0.7), py = s * (0.15 + rng.float() * 0.7);
      const pr = s * (0.035 + rng.float() * 0.025);
      const color = [48 + rng.jitter(15), 95 + rng.jitter(20), 38 + rng.jitter(10)];
      pushCircle3(parts, MATERIALS.flesh(color), px, py, pr, 0.12);
    }
    return parts;
  }
  function buildSpikeTrap(rng, s) {
    const parts = [];
    const base = [78 + rng.jitter(6), 72 + rng.jitter(5), 68 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.bone(base), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.12);
    const cols = 3, rows = 3;
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        pushCircle3(parts, MATERIALS.bone([30, 26, 22]), sx, sy, s * 0.035, 0.1);
        pushCircle3(parts, MATERIALS.metal([160, 155, 148]), sx, sy, s * 0.018, 0.6);
      }
    }
    return parts;
  }
  function buildStairsDown(rng, s) {
    const parts = [];
    const base = [72 + rng.jitter(6), 68 + rng.jitter(5), 64 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.bone(base), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.15);
    const step = MATERIALS.bone([base[0] * 0.8, base[1] * 0.8, base[2] * 0.8]);
    const dark = MATERIALS.bone([20, 18, 16]);
    const steps = 4;
    for (let i = 0; i < steps; i++) {
      const sy = s * (0.2 + i * 0.16);
      const sw = s * (0.38 - i * 0.03);
      pushBox2(parts, step, s * 0.5, sy, sw / 2, s * 0.05, s * 0.01, 0.2);
    }
    pushBox2(parts, dark, s * 0.5, s * 0.82, s * 0.14, s * 0.08, s * 0.02, 0.1);
    return parts;
  }
  function buildStairsUp(rng, s) {
    ...
    const base = [72 + rng.jitter(6), 68 + rng.jitter(5), 64 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.bone(base), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.15);
    const step = MATERIALS.bone([base[0] * 1.1, base[1] * 1.1, base[2] * 1.1]);
    ...
    pushCircle3(parts, light, s * 0.5, s * 0.14, s * 0.06, 0.8);
    return parts;
  }
  function buildCrackedWall(rng, s) {
    const parts = [];
    const base = [58 + rng.jitter(8), 54 + rng.jitter(6), 52 + rng.jitter(6)];
    pushBox2(parts, MATERIALS.bone(base), s * 0.5, s * 0.5, s * 0.48, s * 0.48, s * 0.02, 0.28);
    const edgeDark = [base[0] * 0.5, base[1] * 0.5, base[2] * 0.5];
    pushBox2(parts, MATERIALS.bone(edgeDark), s * 0.97, s * 0.5, s * 0.02, s * 0.48, s * 5e-3, 0.15);
    pushBox2(parts, MATERIALS.bone(edgeDark), s * 0.5, s * 0.97, s * 0.48, s * 0.02, s * 5e-3, 0.15);
    const edgeLight = [base[0] * 1.15, base[1] * 1.15, base[2] * 1.15];
    pushBox2(parts, MATERIALS.bone(edgeLight), s * 0.03, s * 0.5, s * 0.02, s * 0.48, s * 5e-3, 0.18);
    pushBox2(parts, MATERIALS.bone(edgeLight), s * 0.5, s * 0.03, s * 0.48, s * 0.02, s * 5e-3, 0.18);
    const crackCol = MATERIALS.bone([base[0] * 0.35, base[1] * 0.35, base[2] * 0.35]);
    const cracks = 3 + Math.floor(rng.float() * 3);
    for (let i = 0; i < cracks; i++) {
      const ax = s * (0.15 + rng.float() * 0.7), ay = s * (0.15 + rng.float() * 0.7);
      const bx = ax + rng.jitter(s * 0.25), by = ay + rng.jitter(s * 0.25);
      pushCapsule3(parts, crackCol, ax, ay, bx, by, Math.max(1, s * 0.012), 0.1);
    }
    return parts;
  }
  function buildPit(rng, s) {
    const parts = [];
    const edge = [68 + rng.jitter(6), 62 + rng.jitter(5), 58 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.bone(edge), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.12);
    pushCircle3(parts, MATERIALS.bone([15, 12, 10]), s * 0.5, s * 0.5, s * 0.3, 0.1);
    pushCircle3(parts, MATERIALS.bone([edge[0] * 0.7, edge[1] * 0.7, edge[2] * 0.7]), s * 0.5, s * 0.5, s * 0.32, 0.15);
    return parts;
  }
  function buildWaterPool(rng, s) {
    const parts = [];
    const j = rng.jitter(6);
    const base = [30 + j, 55 + j, 70 + j];
    pushBox2(parts, MATERIALS.glass(base), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.04, 0.15);
    ...
    for (let i = 0; i < ripples; i++) {
      ...
      const rippleCol = [base[0] + 35, base[1] + 40, base[2] + 35];
      const thickness = Math.max(1.2, s * 0.012);
      pushCapsule3(parts, MATERIALS.glass(rippleCol), cx - r, cy, cx, cy - r, thickness, 0.2);
      pushCapsule3(parts, MATERIALS.glass(rippleCol), cx, cy - r, cx + r, cy, thickness, 0.2);
      pushCapsule3(parts, MATERIALS.glass(rippleCol), cx + r, cy, cx, cy + r, thickness, 0.2);
      pushCapsule3(parts, MATERIALS.glass(rippleCol), cx, cy + r, cx - r, cy, thickness, 0.2);
    }
    return parts;
  ...
  function buildUndergroundRiver(rng, s) {
    const parts = [];
    const bankCol = [78 + rng.jitter(6), 72 + rng.jitter(5), 68 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.bone(bankCol), s * 0.5, s * 0.12, s * 0.46, s * 0.12, s * 0.02, 0.2);
    pushBox2(parts, MATERIALS.bone(bankCol), s * 0.5, s * 0.88, s * 0.46, s * 0.12, s * 0.02, 0.2);
    const j = rng.jitter(6);
    const waterCol = [25 + j, 50 + j, 65 + j];
    pushBox2(parts, MATERIALS.glass(waterCol), s * 0.5, s * 0.5, s * 0.46, s * 0.28, s * 0.02, 0.12);
    ...
    for (let i = 0; i < ripples; i++) {
      ...
      const rippleCol = [waterCol[0] + 30, waterCol[1] + 35, waterCol[2] + 30];
      pushCapsule3(parts, MATERIALS.glass(rippleCol), ax, ry, bx, ry + rng.jitter(s * 0.02), Math.max(1.2, s * 0.01), 0.18);
    }
    return parts;
  }
  function buildStalagmite(rng, s) {
    const parts = [];
    const baseCol = [78 + rng.jitter(6), 72 + rng.jitter(5), 68 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.bone(baseCol), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.12);
    ...
    for (let i = 0; i < pillars; i++) {
      ...
      pushCircle3(parts, MATERIALS.bone([baseCol[0] * 0.6, baseCol[1] * 0.6, baseCol[2] * 0.6]), cx, cy, baseR + s * 0.02, 0.1);
      pushCircle3(parts, MATERIALS.bone(col), cx, cy, baseR, 0.25);
      pushCircle3(parts, MATERIALS.bone([col[0] + 20, col[1] + 18, col[2] + 15]), cx, cy, baseR * 0.45, 0.4);
    }
    return parts;
  }
  function buildCobweb(rng, s) {
    ...
    for (let i = 0; i < strands; i++) {
      ...
      const thickness = Math.max(1.2, s * 8e-3);
      pushCapsule3(parts, MATERIALS.bone(webCol), originX, originY, ex, ey, thickness, 0.08);
    }
    for (let i = 0; i < 2; i++) {
      ...
      pushCapsule3(parts, MATERIALS.bone(webCol), ax, ay, bx, by, Math.max(1.2, s * 6e-3), 0.06);
    }
    return parts;
  }
  function buildBarrel(rng, s) {
    const parts = [];
    const floorCol = [78 + rng.jitter(6), 72 + rng.jitter(5), 68 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.bone(floorCol), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.12);
    const woodCol = [118 + rng.jitter(10), 78 + rng.jitter(8), 48 + rng.jitter(6)];
    const bandMat = MATERIALS.metal([140, 135, 128]);
    pushCircle3(parts, bandMat, s * 0.5, s * 0.5, s * 0.22, 0.4);
    pushCircle3(parts, MATERIALS.leather(woodCol), s * 0.5, s * 0.5, s * 0.19, 0.3);
    const plankDark = [woodCol[0] * 0.8, woodCol[1] * 0.8, woodCol[2] * 0.8];
    pushCapsule3(parts, MATERIALS.leather(plankDark), s * 0.35, s * 0.32, s * 0.65, s * 0.32, Math.max(1.2, s * 8e-3), 0.2);
    pushCapsule3(parts, MATERIALS.leather(plankDark), s * 0.33, s * 0.5, s * 0.67, s * 0.5, Math.max(1.2, s * 8e-3), 0.2);
    pushCapsule3(parts, MATERIALS.leather(plankDark), s * 0.35, s * 0.68, s * 0.65, s * 0.68, Math.max(1.2, s * 8e-3), 0.2);
    return parts;
  }
  function buildChain(rng, s) {
    const parts = [];
    pushBox2(parts, MATERIALS.bone([22, 20, 18]), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.08);
    ...
    for (let i = 0; i < links; i++) {
      ...
      if (i % 2 === 0) {
        ...
        pushCapsule3(parts, chainMat, cx, ly - linkR * 0.6, cx, ly + linkR * 0.6, Math.max(1.2, s * 0.018), 0.45);
      }
    }
    return parts;
  }
  function buildBonePile(rng, s) {
    ...
    for (let i = 0; i < bones; i++) {
      ...
      pushCapsule3(parts, boneMat, ax, ay, bx, by, thickness, 0.3);
    }
    pushCircle3(parts, boneMat, s * (0.45 + rng.jitter(0.08)), s * (0.45 + rng.jitter(0.06)), s * 0.04, 0.35);
    return parts;
  }
  function buildShopCounter(rng, s) {
    ...
    const floorCol = [78 + rng.jitter(6), 72 + rng.jitter(5), 68 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.bone(floorCol), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.12);
    const woodCol = [105 + rng.jitter(8), 68 + rng.jitter(6), 42 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.leather(woodCol), s * 0.5, s * 0.55, s * 0.38, s * 0.08, s * 0.02, 0.28);
    pushBox2(parts, MATERIALS.leather(woodCol), s * 0.15, s * 0.38, s * 0.08, s * 0.2, s * 0.02, 0.26);
    const edgeLight = [woodCol[0] + 15, woodCol[1] + 10, woodCol[2] + 8];
    pushBox2(parts, MATERIALS.leather(edgeLight), s * 0.5, s * 0.48, s * 0.36, s * 0.012, s * 5e-3, 0.2);
    pushCircle3(parts, MATERIALS.gold([210, 185, 80]), s * 0.58, s * 0.55, s * 0.025, 0.5);
    pushCircle3(parts, MATERIALS.gold([195, 170, 70]), s * 0.45, s * 0.55, s * 0.02, 0.45);
    pushBox2(parts, MATERIALS.leather([woodCol[0] * 0.85, woodCol[1] * 0.85, woodCol[2] * 0.85]), s * 0.7, s * 0.55, s * 0.04, s * 0.03, s * 8e-3, 0.2);
    return parts;
  }
  function buildIronGate(rng, s) {
    const parts = [];
    const frameCol = [52 + rng.jitter(6), 48 + rng.jitter(5), 46 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.bone(frameCol), s * 0.1, s * 0.5, s * 0.1, s * 0.48, s * 0.02, 0.28);
    pushBox2(parts, MATERIALS.bone(frameCol), s * 0.9, s * 0.5, s * 0.1, s * 0.48, s * 0.02, 0.28);
    pushBox2(parts, MATERIALS.bone([72, 66, 62]), s * 0.5, s * 0.5, s * 0.32, s * 0.48, s * 0.02, 0.1);
    const barMat = MATERIALS.metal([120 + rng.jitter(8), 118 + rng.jitter(6), 115 + rng.jitter(6)]);
    const bars = 5;
    for (let i = 0; i < bars; i++) {
      const by = s * (0.18 + i * 0.16);
      pushCapsule3(parts, barMat, s * 0.2, by, s * 0.8, by, Math.max(1.2, s * 0.018), 0.45);
    }
    pushCapsule3(parts, barMat, s * 0.5, s * 0.12, s * 0.5, s * 0.88, Math.max(1.2, s * 0.016), 0.4);
    return parts;
  }
  function buildTorchBracket(rng, s) {
    const parts = [];
    const wallCol = [62 + rng.jitter(8), 58 + rng.jitter(6), 56 + rng.jitter(6)];
    pushBox2(parts, MATERIALS.bone(wallCol), s * 0.5, s * 0.5, s * 0.48, s * 0.48, s * 0.02, 0.28);
    const edgeDark = [wallCol[0] * 0.5, wallCol[1] * 0.5, wallCol[2] * 0.5];
    pushBox2(parts, MATERIALS.bone(edgeDark), s * 0.5, s * 0.97, s * 0.48, s * 0.02, s * 5e-3, 0.15);
    const edgeLight = [wallCol[0] * 1.2, wallCol[1] * 1.2, wallCol[2] * 1.2];
    pushBox2(parts, MATERIALS.bone(edgeLight), s * 0.5, s * 0.03, s * 0.48, s * 0.02, s * 5e-3, 0.18);
    const bracketMat = MATERIALS.metal([130, 125, 120]);
    pushBox2(parts, bracketMat, s * 0.5, s * 0.82, s * 0.04, s * 0.06, s * 0.01, 0.4);
    const torchWood = [100, 62, 35];
    pushCapsule3(parts, MATERIALS.leather(torchWood), s * 0.5, s * 0.82, s * 0.5, s * 0.58, Math.max(1.2, s * 0.022), 0.25);
    pushCircle3(parts, MATERIALS.ember([255, 180, 40]), s * 0.5, s * 0.55, s * 0.06, 0.7);
    pushCircle3(parts, MATERIALS.ember([255, 230, 120]), s * 0.5, s * 0.55, s * 0.03, 0.8);
    return parts;
  }
  function buildAltar(rng, s) {
    const parts = [];
    const floorCol = [68 + rng.jitter(6), 62 + rng.jitter(5), 58 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.bone(floorCol), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.12);
    const stoneCol = [78 + rng.jitter(5), 72 + rng.jitter(4), 82 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.bone([floorCol[0] * 0.6, floorCol[1] * 0.6, floorCol[2] * 0.6]), s * 0.52, s * 0.52, s * 0.22, s * 0.16, s * 0.02, 0.08);
    pushBox2(parts, MATERIALS.bone(stoneCol), s * 0.5, s * 0.5, s * 0.2, s * 0.14, s * 0.03, 0.25);
    pushBox2(parts, MATERIALS.bone([stoneCol[0] + 18, stoneCol[1] + 15, stoneCol[2] + 20]), s * 0.5, s * 0.37, s * 0.18, s * 0.012, s * 5e-3, 0.2);
    const runeCol = [140 + rng.jitter(20), 80 + rng.jitter(15), 200 + rng.jitter(20)];
    pushCircle3(parts, MATERIALS.gem(runeCol), s * 0.5, s * 0.5, s * 0.08, 0.6);
    pushCircle3(parts, MATERIALS.ember([180, 120, 255]), s * 0.5, s * 0.5, s * 0.04, 0.8);
    return parts;
  }
  function buildAnvil(rng, s) {
    const parts = [];
    const floorCol = [62 + rng.jitter(6), 56 + rng.jitter(5), 52 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.bone(floorCol), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.12);
    const anvilCol = [55 + rng.jitter(6), 52 + rng.jitter(5), 50 + rng.jitter(5)];
    pushEllipse3(parts, MATERIALS.bone([floorCol[0] * 0.55, floorCol[1] * 0.55, floorCol[2] * 0.55]), s * 0.47, s * 0.52, s * 0.2, s * 0.1, 0.08);
    pushBox2(parts, MATERIALS.metal(anvilCol), s * 0.5, s * 0.5, s * 0.06, s * 0.12, s * 0.015, 0.35);
    const topCol = [anvilCol[0] + 18, anvilCol[1] + 15, anvilCol[2] + 12];
    pushBox2(parts, MATERIALS.metal(topCol), s * 0.5, s * 0.42, s * 0.16, s * 0.05, s * 0.02, 0.4);
    pushCapsule3(parts, MATERIALS.metal(anvilCol), s * 0.5, s * 0.58, s * 0.5, s * 0.7, Math.max(1.2, s * 0.025), 0.38);
    const hammerHead = [100, 95, 90];
    pushBox2(parts, MATERIALS.metal(hammerHead), s * 0.72, s * 0.42, s * 0.035, s * 0.025, s * 8e-3, 0.4);
    pushCapsule3(parts, MATERIALS.leather([90, 58, 35]), s * 0.72, s * 0.46, s * 0.72, s * 0.62, Math.max(1.2, s * 0.012), 0.22);
    return parts;
  }
  function buildBed(rng, s) {
    ...
    const floorCol = [78 + rng.jitter(6), 72 + rng.jitter(5), 68 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.bone(floorCol), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.12);
    const frameCol = [95 + rng.jitter(8), 62 + rng.jitter(6), 38 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.leather(frameCol), s * 0.5, s * 0.5, s * 0.2, s * 0.35, s * 0.02, 0.2);
    pushBox2(parts, MATERIALS.leather([frameCol[0] * 0.85, frameCol[1] * 0.85, frameCol[2] * 0.85]), s * 0.5, s * 0.16, s * 0.22, s * 0.03, s * 0.015, 0.25);
    const blanketCol = [65 + rng.jitter(15), 55 + rng.jitter(10), 80 + rng.jitter(15)];
    pushBox2(parts, MATERIALS.cloth(blanketCol), s * 0.5, s * 0.55, s * 0.17, s * 0.25, s * 0.02, 0.15);
    const pillowCol = [160 + rng.jitter(10), 155 + rng.jitter(8), 145 + rng.jitter(8)];
    pushEllipse3(parts, MATERIALS.cloth(pillowCol), s * 0.5, s * 0.24, s * 0.1, s * 0.05, 0.2);
    return parts;
  }
  function buildTable(rng, s) {
    ...
    const floorCol = [78 + rng.jitter(6), 72 + rng.jitter(5), 68 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.bone(floorCol), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.12);
    const topCol = [110 + rng.jitter(8), 72 + rng.jitter(6), 45 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.bone([floorCol[0] * 0.6, floorCol[1] * 0.6, floorCol[2] * 0.6]), s * 0.52, s * 0.52, s * 0.28, s * 0.18, s * 0.02, 0.08);
    pushBox2(parts, MATERIALS.leather(topCol), s * 0.5, s * 0.5, s * 0.26, s * 0.16, s * 0.03, 0.28);
    pushBox2(parts, MATERIALS.leather([topCol[0] + 12, topCol[1] + 8, topCol[2] + 6]), s * 0.5, s * 0.35, s * 0.24, s * 0.012, s * 5e-3, 0.2);
    pushCircle3(parts, MATERIALS.bone([140, 130, 115]), s * 0.6, s * 0.48, s * 0.03, 0.3);
    pushCircle3(parts, MATERIALS.bone([95, 80, 65]), s * 0.6, s * 0.48, s * 0.018, 0.15);
    pushCircle3(parts, MATERIALS.bone([165, 158, 145]), s * 0.38, s * 0.5, s * 0.04, 0.2);
    return parts;
  }
  function buildBookshelf(rng, s) {
    ...
    const edgeDark = [wallCol[0] * 0.5, wallCol[1] * 0.5, wallCol[2] * 0.5];
    pushBox2(parts, MATERIALS.bone(edgeDark), s * 0.5, s * 0.97, s * 0.48, s * 0.02, s * 5e-3, 0.15);
    const shelfCol = [100 + rng.jitter(8), 65 + rng.jitter(6), 38 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.leather(shelfCol), s * 0.5, s * 0.75, s * 0.38, s * 0.1, s * 0.02, 0.22);
    const bookColors = [
    ...
    for (let i = 0; i < 4; i++) {
      ...
      pushBox2(parts, MATERIALS.cloth(bookColors[i]), bx, s * 0.75, s * 0.01, s * 0.06, s * 4e-3, 0.15);
    }
    return parts;
  }
  function buildPillar(rng, s) {
    ...
    const floorCol = [78 + rng.jitter(6), 72 + rng.jitter(5), 68 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.bone(floorCol), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.12);
    const pillarCol = [105 + rng.jitter(8), 98 + rng.jitter(6), 90 + rng.jitter(6)];
    pushCircle3(parts, MATERIALS.bone([floorCol[0] * 0.55, floorCol[1] * 0.55, floorCol[2] * 0.55]), s * 0.52, s * 0.52, s * 0.18, 0.08);
    pushCircle3(parts, MATERIALS.bone([pillarCol[0] - 10, pillarCol[1] - 8, pillarCol[2] - 6]), s * 0.5, s * 0.5, s * 0.16, 0.25);
    pushCircle3(parts, MATERIALS.bone(pillarCol), s * 0.5, s * 0.5, s * 0.12, 0.35);
    pushCircle3(parts, MATERIALS.bone([pillarCol[0] + 18, pillarCol[1] + 15, pillarCol[2] + 12]), s * 0.48, s * 0.47, s * 0.05, 0.3);
    return parts;
  }
  function buildFountain(rng, s) {
    ...
    const floorCol = [78 + rng.jitter(6), 72 + rng.jitter(5), 68 + rng.jitter(5)];
    pushBox2(parts, MATERIALS.bone(floorCol), s * 0.5, s * 0.5, s * 0.46, s * 0.46, s * 0.02, 0.12);
    const basinCol = [88 + rng.jitter(6), 82 + rng.jitter(5), 78 + rng.jitter(5)];
    pushCircle3(parts, MATERIALS.bone(basinCol), s * 0.5, s * 0.5, s * 0.3, 0.22);
    pushCircle3(parts, MATERIALS.bone([basinCol[0] * 0.6, basinCol[1] * 0.6, basinCol[2] * 0.6]), s * 0.5, s * 0.5, s * 0.24, 0.15);
    const waterCol = [40 + rng.jitter(8), 75 + rng.jitter(10), 110 + rng.jitter(10)];
    pushCircle3(parts, MATERIALS.glass(waterCol), s * 0.5, s * 0.5, s * 0.2, 0.2);
    pushCircle3(parts, MATERIALS.bone([basinCol[0] + 12, basinCol[1] + 10, basinCol[2] + 8]), s * 0.5, s * 0.5, s * 0.04, 0.3);
    const rippleCol = [waterCol[0] + 30, waterCol[1] + 35, waterCol[2] + 30];
    const thickness = Math.max(1.2, s * 0.01);
    const r1 = s * 0.1;
    pushCapsule3(parts, MATERIALS.glass(rippleCol), s * 0.5 - r1, s * 0.5, s * 0.5, s * 0.5 - r1, thickness, 0.18);
    pushCapsule3(parts, MATERIALS.glass(rippleCol), s * 0.5, s * 0.5 - r1, s * 0.5 + r1, s * 0.5, thickness, 0.18);
    pushCapsule3(parts, MATERIALS.glass(rippleCol), s * 0.5 + r1, s * 0.5, s * 0.5, s * 0.5 + r1, thickness, 0.18);
    pushCapsule3(parts, MATERIALS.glass(rippleCol), s * 0.5, s * 0.5 + r1, s * 0.5 - r1, s * 0.5, thickness, 0.18);
    return parts;
  }
  ...
  function buildLootBag(rng, s, color) {
    ...
    pushEllipse4(parts, sack, cx, s * 0.58, s * 0.22, s * 0.18, 0.9);
    pushEllipse4(parts, sack, cx, s * 0.42, s * 0.12, s * 0.1, 0.8);
    pushCapsule4(parts, tie, cx - s * 0.06, s * 0.38, cx + s * 0.06, s * 0.38, Math.max(1, s * 0.015), 0.5);
    pushCircle4(parts, tie, cx, s * 0.36, s * 0.025, 0.7);
    return parts;
  }
  ...
  function buildGravestone(rng, s) {
    ...
    pushBox3(parts, stone, cx, s * 0.48, s * 0.18, s * 0.28, s * 0.04, 0.35);
    pushCircle4(parts, stone, cx, s * 0.22, s * 0.18, 0.4);
    pushBox3(parts, MATERIALS.flesh([70, 55, 38]), cx, s * 0.8, s * 0.24, s * 0.06, s * 0.02, 0.2);
    pushCapsule4(
      parts,
      dark,
