...
function addTall(kind:string, seed:string, tx:number, ty:number, w:number, hMul:number){
  // bare: true so props that paint their own floor slab don't show it as a
  // hard square over the grass (none of the current kinds used here do,
  // but this keeps the helper safe for whatever gets added next).
